...
//将求解出的数据进行储存（这里好像是将数据映射回去了给覆盖住了）
...
{
    BALProblem bal_problem(filename);     //将文件中的数据全部读取并分类号
    ...
    bal_problem.Normalize();            //对当前类中数据归一化
    ...
    SetSolverOptionsFromFlags(&bal_problem, params, &optimizer);   //按照自己定义的参数对g2o进行设置
    BuildProblem(&bal_problem, &optimizer, params);   //将数据添加入g2o中
