...
public:
    ...
    {
        Eigen::VectorXd::ConstMapType v ( update, VertexCameraBAL::Dimension ); //多维动态映射
        _estimate += v;      //? 如何理解累加的更新
    }
...
public:
    ...
    virtual void oplusImpl ( const double* update )
    {
        Eigen::Vector3d::ConstMapType v ( update ); //低维
        _estimate += v;
    }
};
...
public:
    ...
    virtual void computeError() override   // The virtual function comes from the Edge base class. Must define if you use edge.
    {
        ...
        ( *this ) ( cam->estimate().data(), point->estimate().data(), _error.data() );    //核心在这里，这里用了下面的括号的重定义（所以这里将原始数据输入了）
